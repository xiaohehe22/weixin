...
回复【报名】，即可参与免费的班级旅行，回复【规则】，查看详细攻略";
	...
	private function receiveText($object) {
		...
		switch (substr ( $keyword, 0, 6 )) {            
            ...
            break;
             case "听歌":
                $content = "想听歌？<a href=\"http://www.ximalaya.com/zhubo/42585365/\">戳这里</a>";
                 $result = $this->transmitText ( $object, $content );
            break;
                case "沙龙":
				 $content = "1. 将下 面海报分享至朋友圈，并附上【报名参加】字样\n2. 扫描二维码加小师兄为好友，将分享截图发送给小师兄\n3. 等待小师兄拉群\n4. 因为人数较多，所以请不要多次发送截图，请耐心等待\n5. 进群同学请按群公告填写报名表，小师兄将按报名信息发送邀请函，务必真实可靠";
                 $result = $this->transmit2( $object, "uI1m7w1-9gdGALsNb3tC7UmA_--dgQM6fBh9INYA0-E" );
            break;
            default :
...
$item_str</Articles>
	...
	// 回复图片消息
	private function transmitImage($object,$content) { // irU924xhACd43a5fnQRfKbJ29eJGpHZAzLWfYfxZdWSoKtKSWxVzbOv3yTReDG3X
		$itemTpl = "<Image>
...
$item_str
		...
		$result1 = sprintf ( $textTpl, $fromUsername, $toUsername, time () );
        ...
        if($result2!=null){
            ...
            $time = $result2[0]["time"]; 
            if(time() - $time <7000){
                $this->post_text($access,$fromUsername,'85YhtdfSqcDC3MwQmplPFNO3jtpDVKyb_fpc3HlsHi0');
               return $result1;            
            }
        } 
		...
		$mysql->runSql($sql);           
		$mysql->closeDb();
        $this->post_text($this_token,$fromUsername,'85YhtdfSqcDC3MwQmplPFNO3jtpDVKyb_fpc3HlsHi0');   
